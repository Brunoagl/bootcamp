...
void animar_personaje(char **laberinto, int filas, int columnas,int *camino, int path_len, int pausa_ms,
    ...
    for (int i = 0; i < path_len; i++) { // Bucle que recorre cada paso del camino
        ...
        imprimir_laberinto(laberinto, filas, columnas, pausa_ms, camino_fila, camino_columna); // imprime el laberinto
    }

    // Marcar la mejor ruta con "*"
    for (int i = 0; i < path_len; i++) { // Recorre tdas las posiciones del camino  
        ...
        if (!(fila_actual == inicio_i && columna_actual == inicio_j) && !(fila_actual == salida_i && columna_actual == salida_j)) {
            laberinto[fila_actual][columna_actual] = RUTA; // marca las casilla con * excepto la casilla de inicio y la de salida.
        }
    }
...
int main() {
    ...
    char buf[64]; // lee el texto y concierte en numero
    ...
    fflush(stdout);
    if (fgets(buf, sizeof(buf), stdin) == NULL || buf[0] == '\n') { // si no se escribio nada o hubo un error se usa 10
        filas = 10;
    } else {
        filas = atoi(buf); // convierte texto a numeros
    }

    // Pedir columnas
    printf("Ingrese columnas (>=5, impar) [enter = 10]: "); // lo mismo
    fflush(stdout);
    if (fgets(buf, sizeof(buf), stdin) == NULL || buf[0] == '\n') { // lo mismo
        columnas = 10;
    } else {
        columnas = atoi(buf); // lo mismo
    }
    ...
    // Crear matriz dinámica y llena con pared
    ...
    // Generar laberinto
    clock_t start_gen = clock(); // guarda el tiempo de generacion
    laberinto[1][1] = CAMINO; // Abre un camino y llama a la función que se encarga de crear el laberinto
    generar_laberinto(laberinto, 1, 1, filas, columnas);
    clock_t end_gen = clock(); // guarda el tiempo en q termina la generacion

    // Marcar inicio y salida
    int inicio_i = 1, inicio_j = 1; // defino el inicio
    int salida_i = filas - 2, salida_j = columnas - 2; // defino la salida
    laberinto[inicio_i][inicio_j] = INICIO; // marco en el tablero I
    laberinto[salida_i][salida_j] = SALIDA; // marco en el trablero S

    imprimir_laberinto(laberinto, filas, columnas, 800, -1, -1); // imprimo el laberinto actualizado

    // Buscar camino con BFS
    int *camino = NULL; int path_len = 0; // preparo un puntero para guardar el camino encontrado y una variable para su longitud
    clock_t start_bfs = clock(); // guarda el tiempo en q empieza la busqueda
    if (bfs_find_path(laberinto, filas, columnas, inicio_i, inicio_j, salida_i, salida_j, &camino, &path_len)) { // busca la ruta de I a S
        clock_t end_bfs = clock();
        printf("Ruta encontrada. Longitud: %d pasos\n", path_len); // muestra la cantidad de pasos
        printf("Tiempo BFS: %.4f segundos\n", // muestra el tiempo que tardó en buscar
               (double)(end_bfs - start_bfs) / CLOCKS_PER_SEC);

        animar_personaje(laberinto, filas, columnas, camino, path_len, 180, inicio_i, inicio_j, salida_i, salida_j); // animo el reccorrido
        free(camino); // libero la memoria usada 
    } else { // si no encontro ruta
        clock_t end_bfs = clock(); // igual mide el tiempo
        printf("No se encontró ruta.\n"); // marca q no encontro ruta
        printf("Tiempo BFS: %.4f segundos\n", // marca el tiempo
               (double)(end_bfs - start_bfs) / CLOCKS_PER_SEC);
    }
    ...
    free(laberinto); // libero memoria del laberinto

    clock_t end_total = clock(); // Marca el tiempo en que terminó todo el programa
